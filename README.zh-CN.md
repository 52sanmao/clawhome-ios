...
ClawHome iOS 是一个轻量级的 IronClaw 控制端。
它优先连接 IronClaw HTTP 端点，核心能力是会话列表与聊天 UI 展示。
...
当你已经有兼容 IronClaw 的服务地址时，ClawHome iOS 提供一个原生 iOS 控制面板来进行会话查看与交互。

旧版 OpenClaw 的各类 Channel（如 WhatsApp/Telegram 等）在某些兼容部署里仍可能运行于该服务后端。
ClawHome 不承载 Channel 运行时，主要连接该服务的 HTTP API 控制面。
因此 ClawHome 能控制整套部署，但前提是你的 iPhone 能访问到服务地址。
...
随后输出可被 ClawHome 直接识别的服务地址，并在终端渲染二维码。
...
- `clawhome/Core/Network/`：IronClaw 兼容网络层
...
- 无法连接服务：
  - 确认服务地址是可访问的 `http://` 或 `https://` IronClaw 端点
  - 确认设备到服务主机/端口网络可达
  - 如使用鉴权，确认 Bearer Token 或网关密钥正确
- 局域网配对超时：
  - 你的服务大概率仍是 `loopback` 绑定
  - 使用你的部署方式重新绑定到 LAN 后再生成二维码
- Cloudflare 配对失败：
  - 安装 `cloudflared` 后重试：
